...
function getdateActual($fecha_ingreso)
{
     ...
     if ($f_ingreso_actual < $today) {
         $a = $f_ingreso_actual->toDateTimeString();
     }
     else{
         $a = $f_ingreso_ano_anterior->toDateTimeString();
     }
...
function getdatePosterior($date)
{
     ...
     return $dt->addYear()->subDay()->toDateTimeString();
}
...
function get_departamento($deparment_id)  {
    ...
    $dpto = Deparment::find($deparment_id);
    if  ($dpto->code == "00104")
        return "00105";
    else
        return  $dpto->code;
